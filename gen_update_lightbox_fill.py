    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 select-none">
          ...
          {/* Large Immersive Modal (92vw width, 90vh height) */}
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 15 }}
            transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
            className="relative w-[92vw] max-w-6xl h-[88vh] bg-[#120407] border border-white/20 rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.95)] overflow-hidden flex flex-col z-10"
          >
            {/* Header Bar */}
            ...
            {/* Main Stage: Image fills at least 3/4 of the modal expanse */}
            <div className="relative flex-1 w-full h-full bg-black/80 flex items-center justify-center overflow-hidden p-3 sm:p-6 group">
              {currentPhoto && (
                <div className="w-full h-full flex items-center justify-center">
                  <motion.img
                    ...
                    className="w-auto h-auto max-w-[85vw] max-h-[66vh] md:max-h-[68vh] object-contain rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-white/10 mx-auto"
                  />
                </div>
              )}
              ...
              {totalCount > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-3 sm:left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/75 hover:bg-[#7A1220] text-white border border-white/25 flex items-center justify-center transition-all shadow-2xl hover:scale-110 active:scale-95 z-20"
                  >
                  ...
                  <button
                    ...
                    className="absolute right-3 sm:right-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/75 hover:bg-[#7A1220] text-white border border-white/25 flex items-center justify-center transition-all shadow-2xl hover:scale-110 active:scale-95 z-20"
                  >
...
""")
print("BranchPhotoLightbox.tsx updated to fill at least 3/4 of the modal!")
